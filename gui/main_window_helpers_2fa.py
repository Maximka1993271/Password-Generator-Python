"""
Main window helper methods - 2FA operations
Методы-помощники главного окна - 2FA операции
Методи-помічники головного вікна - 2FA операції
...
logger = get_logger("main_window_helpers_2fa")
...
class MainWindow2FAMixin:
    ...
    def _setup_2fa(self) -> None:
        ...
        if self.config.is_2fa_enabled():
            result: Union[bool, str] = CTkMessageBox.question(
                self,
                L.get("2fa_title", "Two-Factor Authentication"),
        ...
        account_name: str = self.config.get_2fa_account_name()
        if not account_name:
            account_name = "SecurePassPro_User"

        totp: TOTP = TOTP()
        secret: str = totp.secret
        provisioning_uri: str = TOTP.get_provisioning_uri(secret, account_name, "SecurePassPro")
        ...
        code: Optional[str] = self._verify_2fa_dialog(
        ...
        code_clean: str = ''.join(filter(str.isdigit, code))

        if totp.verify(code_clean)[0]:
            backup_codes: List[str] = totp.get_backup_codes(count=10, length=8)
    ...
    def _disable_2fa(self) -> None:
        ...
        secret: str = self.config.get_2fa_secret()
        if secret:
            secret_clean: str = secret.upper().replace(" ", "").replace("-", "")
            totp: TOTP = TOTP(secret_clean)
            if totp.verify(code_clean)[0]:
                manager = get_totp_manager()
                manager.disable_2fa()
    ...
    def _verify_2fa_dialog(self, title: str, prompt: str) -> Optional[str]:
        ...
        L: Dict[str, str] = LANGUAGES[self.current_lang]
        theme: str = self._get_actual_theme()
        ...
        x: int = self.winfo_x() + (self.winfo_width() - 450) // 2
        y: int = self.winfo_y() + (self.winfo_height() - 350) // 2
        ...
        main_frame: ctk.CTkFrame = ctk.CTkFrame(dialog, fg_color="transparent")
    ...
    def _show_backup_codes(self, backup_codes: List[str]) -> None:
        """Show backup codes in a separate window."""
        L: Dict[str, str] = LANGUAGES[self.current_lang]
        theme: str = self._get_actual_theme()
        radius: int = get_global_radius()

        win: ctk.CTkToplevel = ctk.CTkToplevel(self)
        ...
        x: int = self.winfo_x() + (self.winfo_width() - 550) // 2
        y: int = self.winfo_y() + (self.winfo_height() - 600) // 2
        ...
        ctk.CTkLabel(
            main_frame,
            text=L.get("2fa_backup_codes", "Backup Codes / Резервные коды / Резервні коди"),
            font=("Segoe UI", 20, "bold")
        ).pack(pady=(0, 15))

        warning_frame: ctk.CTkFrame = ctk.CTkFrame(main_frame, fg_color="#3a2a0a", corner_radius=12)
        ...
        codes_frame: ctk.CTkFrame = ctk.CTkFrame(main_frame, fg_color="transparent")
        codes_frame.pack(pady=10)

        left_frame: ctk.CTkFrame = ctk.CTkFrame(codes_frame, fg_color="transparent")
        left_frame.pack(side="left", padx=15)

        right_frame: ctk.CTkFrame = ctk.CTkFrame(codes_frame, fg_color="transparent")
        right_frame.pack(side="left", padx=15)

        half: int = len(backup_codes) // 2 + len(backup_codes) % 2
        for i, code in enumerate(backup_codes):
            target: ctk.CTkFrame = left_frame if i < half else right_frame
            code_text: str = code if '-' in code else f"{code[:4]}-{code[4:]}"
        ...
        copy_btn: ctk.CTkButton = ctk.CTkButton(
            main_frame,
            text=L.get("copy_all", "Copy All / Копировать все / Копіювати всі"),
            command=copy_all_codes,
            width=180,
            height=40,
            fg_color="#2d6a4f",
    ...
    def _show_2fa_settings(self) -> None:
        ...
        is_enabled: bool = self.config.is_2fa_enabled()
        ...
        win.update_idletasks()
        x: int = self.winfo_x() + (self.winfo_width() - 500) // 2
        y: int = self.winfo_y() + (self.winfo_height() - 550) // 2
        ...
        if is_enabled:
            status_text: str = L.get("2fa_status_enabled", "Enabled / Включена / Увімкнено")
            status_color: str = "#2ECC71"
        else:
        ...
        if is_enabled:
            manager = get_totp_manager()
            backup_count: int = manager.get_backup_codes_count()

            ctk.CTkLabel(
                main_frame,
                text=L.get("2fa_backup_codes_info", "Backup Codes / Резервные коды / Резервні коди"),
                font=("Segoe UI", 17, "bold")
            ).pack(anchor="w", pady=(15, 8))

            ctk.CTkLabel(
                main_frame,
                text=L.get("2fa_backup_count", "Remaining backup codes: {0} / Осталось резервных кодов: {0} / Залишилось резервних кодів: {0}").format(backup_count),
                font=("Segoe UI", 13)
                    ...
                    if code:
                        ...
                        if secret:
                            ...
                            if totp.verify(code)[0]:
                                new_codes: List[str] = totp.get_backup_codes(count=10, length=8)
                                manager = get_totp_manager()
            ...
            ctk.CTkButton(
                main_frame,
                text=L.get("2fa_regenerate_backup", "Generate New Backup Codes"),
                command=generate_new_backup,
                width=320,
                height=45,
                fg_color="#FF9800",
                corner_radius=radius,
            ...
            ctk.CTkButton(
                main_frame,
                text=L.get("2fa_disable", "Disable 2FA"),
            ...
            ctk.CTkButton(
                main_frame,
                text=L.get("2fa_enable", "Enable 2FA"),
