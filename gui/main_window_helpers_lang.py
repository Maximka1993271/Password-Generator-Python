...
logger = get_logger("main_window_helpers_lang")
...
class MainWindowLangMixin:
    ...
    def _apply_lang(self, lang: str) -> None:
        ...
        # Update menu buttons
        menu_btns: List[ctk.CTkButton] = [
            self.btn_gen, self.btn_name_gen, self.btn_passphrase,
            self.btn_copy, self.btn_save, self.btn_open,
            ...
            if btn and btn.winfo_exists():
                try:
                    if hasattr(btn, 'icon') and btn.icon:
                        display_text: str = f" {btn.icon}   {L[btn.lang_key]}"
                    else:
        ...
        # Update eye button tooltip
        if "btn_eye" in self._tooltips and self._tooltips["btn_eye"]:
            try:
                self._tooltips["btn_eye"].set_text(
                    L.get("tt_eye", "Show / hide password / Показать / скрыть пароль / Показати / приховати пароль")
                )
        ...
        if hasattr(self, 'btn_eye') and self.btn_eye and self.btn_eye.winfo_exists():
            try:
                ...
                logger.debug(f"Eye button language update error: {e}")
        ...
        if hasattr(self, '_2fa_indicator_label') and self._2fa_indicator_label and self._2fa_indicator_label.winfo_exists():
            try:
                ...
                logger.debug(f"2FA indicator language update error: {e}")
    ...
    def _update_settings_window_language(self) -> None:
        ...
        tab_labels: Dict[str, str] = {
            ...
            "cloud": L.get("tab_cloud", "Облако / Cloud / Хмара"),
        }
    ...
    def _update_settings_widgets_text(self) -> None:
        ...
        def update_widgets_recursive(widget: ctk.CTkBaseClass) -> None:
            try:
