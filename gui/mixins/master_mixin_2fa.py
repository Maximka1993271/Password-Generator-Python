...
# master_mixin_2fa.py
"""
Master mixin 2fa module for Secure Pass Pro.
Модуль Master mixin 2fa для Secure Pass Pro.
Модуль Master mixin 2fa для Secure Pass Pro.
"""
"""
Master mixin 2fa module for Secure Pass Pro.
...
"""
"""
Master password mixin - 2FA methods
Миксин мастер-пароля - Методы 2FA
Міксин майстер-пароля - Методи 2FA
"""
...
class MasterMixin2FA:
    """2FA methods for master password mixin

    Методы 2FA для миксина мастер-пароля
    Методи 2FA для міксина майстер-пароля
    """
    ...
    def _update_2fa_buttons(self) -> None:
        """
        Update 2FA button states in settings

        Обновляет состояние кнопок 2FA в настройках
        Оновлює стан кнопок 2FA в налаштуваннях
        """
        try:
            if hasattr(self, '_2fa_status_label') and self._2fa_status_label and self._2fa_status_label.winfo_exists():
                ...
                if self.config.is_2fa_enabled():
                    self._2fa_status_label.configure(
                        text=L.get("2fa_status_enabled", "Enabled / Включена / Увімкнено"),
                        text_color="#2ECC71"
                    )
                else:
                    self._2fa_status_label.configure(
                        text=L.get("2fa_status_disabled", "Disabled / Отключена / Вимкнено"),
                        text_color="#FF4444"
                    )
        except (tk.TclError, AttributeError, KeyError, RuntimeError) as e:
            logger.debug(f"Update 2FA buttons error / Ошибка обновления кнопок 2FA / Помилка оновлення кнопок 2FA: {e}")
