...
Falls back to XML parser for .xml files if pykeepass is not installed.

Signature matches other importers:
    import_from_kdbx(file_path, parent, lang) -> (imported: int, skipped: int)
"""
...
try:
    ...
    import sys as _sys
    for _m in ('pdbpp', 'pdbpp_utils', 'fancycompleter'):
        if _m not in _sys.modules:
            try:
                _sys.modules[_m] = type(_sys)(_m)   # empty module stub
            except Exception:
                pass
...
def _ask_kdbx_password(parent, lang: str) -> str:
    """
    Show a small dialog to ask for the KeePass database password.
    Показывает диалог для ввода пароля от базы KeePass.
    Показує діалог для введення пароля від бази KeePass.
    Returns the password string, or '' for no password, or None if cancelled.
    """
    labels = {
        "RU": {
    ...
    result = {"value": None}  # None = cancelled, "" = empty (no password)
...
def import_from_kdbx(file_path: str, parent=None, lang: str = "RU") -> Tuple[int, int]:
    """
    Import passwords from a KeePass .kdbx file.

    Импортирует пароли из файла KeePass .kdbx.
    Імпортує паролі з файлу KeePass .kdbx.
    ...
    # Ask for database password (allow up to 3 attempts)
    db_password = None
    ...
    for entry in kp.entries:
        try:
            ...
            # ── save with all v3 fields ───────────────────────────────
            try:
                PasswordDB.save_extended(
                    label=label,
                    password=password,
                    notes=notes,
                    lang=lang,
                    url=url,
                    username=username,
                    email="",           # KeePass has no dedicated email field
                    category=category,
                )
                imported += 1
    ...
    logger.info(
        f"KDBX import: {imported} imported, {skipped} skipped from '{os.path.basename(file_path)}'"
    )
    return imported, skipped


__all__ = ["import_from_kdbx", "PYKEEPASS_AVAILABLE"]
